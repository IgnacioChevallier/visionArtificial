...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # carpeta machine/

# Escribo los valores de los momentos de Hu en el archivo
def write_hu_moments(label, writer):
    files = glob.glob(os.path.join(BASE_DIR, 'shapes', label, '*'))  # ← ruta absoluta
    hu_moments = []
    for file in files:
...
def generate_hu_moments_file():
    os.makedirs(os.path.join(BASE_DIR, 'generated-files'), exist_ok=True)  # ← crea la carpeta si no existe
    with open(os.path.join(BASE_DIR, 'generated-files', 'shapes-hu-moments.csv'), 'w', newline='') as file:
        ...
        write_hu_moments("spade", writer)
        write_hu_moments("heart", writer)
        write_hu_moments("diamond", writer)
        write_hu_moments("club", writer)
